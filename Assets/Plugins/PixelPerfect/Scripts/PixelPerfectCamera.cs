...
public class PixelPerfectCamera : MonoBehaviour
{
    ...
    public Camera normalCamera { get { if (camera_ == null) { camera_ = GetComponent<Camera>(); } return camera_; } }
    Camera camera_;
    ...
    public static Vector3 mouseToPixelCamera(Vector3 mousePosition)
    {
        Vector3 falseLocation = Camera.main.ScreenToWorldPoint(mousePosition);
        Vector3 cameraLocation = Camera.main.transform.position;
        Vector3 realLocation;

        if (GameObject.Find("Main Camera").GetComponent<PixelPerfectCamera>().pixelatedPostProcessing == false)
        {
            return falseLocation;
        }

        int zoomSize = GameObject.Find("Main Camera").GetComponent<PixelPerfectCamera>().cameraZoom;

        realLocation.x = (falseLocation.x - cameraLocation.x) / zoomSize + cameraLocation.x;
        realLocation.y = (falseLocation.y - cameraLocation.y) / zoomSize + cameraLocation.y;
        realLocation.z = 0.0f;
    ...
    {
        Gizmos.color = GetParallaxLayerColor(parallaxLayerIndex) * 0.2f;
