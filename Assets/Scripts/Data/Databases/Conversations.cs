...
namespace Data.Databases {
	...
	public class Conversations: Table
	{
		public Conversations(Database db) : base(db) {}

		// Prerequisites for a single conversation, 0 is always empty
		public int characterLimit { get { return GameSettings.conversationCharacterLimit; } }

		// Finds the single response for a character and responds with a singleConversation
		public void FindAndUpdateConversationForCharacter(Character characterTable, ConversationState conversationStateToUpdate)
		{
			string table = "Conversations_" + characterTable.name;
			table = table.Replace(" ","");

			// Select all conversations with no prerequisites
			ExecuteSQL("SELECT * FROM `" + table + @"` WHERE StateName LIKE 'S%' OR StateName LIKE 's%'");
		...
		public void UpdateConversationForCharacter(string stateName, Character characterTable, ConversationState conversationStateToUpdate)
		{
			string table = "Conversations_" + characterTable.name;
			table = table.Replace(" ","");

			// Execute the query
			ExecuteSQL("SELECT * FROM " + table + @" WHERE StateName LIKE '" + stateName + "'");

			// Fill in the information with the Reader row
			UpdateStateWithRow(Reader, conversationStateToUpdate);
		}

		void UpdateStateWithRow(IDataReader Reader, ConversationState s)
		{
			Reader.Read();
			if (Reader.IsDBNull(0)) return;
			s.stateName = Reader.GetString(0);
			s.addStates = StringToStringList(Reader.GetString(1));
