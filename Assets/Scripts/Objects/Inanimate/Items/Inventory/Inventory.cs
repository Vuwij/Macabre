...
namespace Objects.Items.Inventory {
	...
	public class Inventory {
        ...
        public Inventory(int classALimit_ = 0, int classBLimit_ = 0)
        {
            classALimit = classALimit_;
            classBLimit = classBLimit_;
        }
		...
		public virtual bool Add(Item item) {
			InventoryItem i = (InventoryItem) item;
		...
		/// </summary>
		protected virtual void AddObject(InventoryItem i) {
			foreach(Item m in i.itemList) {
				try {
		...
		public virtual void Drop(InventoryItem i, Transform dropLocation) {
